...
export const shareReport = async (request, response) => {
  const { id } = request.params;
  ...
  try {
    ...
    report.sharedWithCourt = true;
    await report.save();
    return response.status(200).json({ message: "Report shared successfully" });
  } catch (error) {
    console.error("Error sharing report:", error);
    return response.status(500).json({ message: "Error sharing report" });
  }
};

export const getAllSharedReports = async (req, res) => {
  try {
    let reports;
    reports = await Report.find({ sharedWithCourt: true });
